...
私有插件：在 **设置** 侧栏新增独立的「私有插件」入口（与「插件市场」并列），把 **从自己的私有 Git 仓库安装插件** 与 **本地插件导入** 放在同一个界面，并附带已安装插件的查看/移除。

- 导入方式：**私有仓库** 与 **本地文件** 合并为一个「导入方式」入口（只是两种导入渠道，不再在导入区列举插件）——添加 Git 仓库（公开/私有）、上传 `.tgz / .tar.gz / .tar` 压缩包、或选择本地插件文件夹；已保存的仓库仍可在此移除。
- 已安装插件：**统一的插件管理列表**（参考插件市场）——本地导入与从私有仓库安装的云端插件全部在这里展示，每个插件一行提供：
  - **启用开关**：写入 profile 的 `cordis.patch.yml` 补丁层（与手写编辑同一机制），重启后生效；
  - **安装 / 已安装** 按钮：未安装（已保存但未安装的仓库）显示「安装」，已安装显示「已安装」；
  - **更新 / 已更新** 按钮：有新版显示「更新」，刚更新完显示「已更新」；
  - **卸载** 按钮。
  - 未安装的已保存仓库也出现在该列表（带「安装」按钮），来源用「本地 / 仓库」标签区分。
...
- **本地来源**（`file:` / `link:`）：不检查。
- 更新完成后该插件标记为「已更新」，顶部状态条提示重启 Harness（桌面版可一键重启）。
