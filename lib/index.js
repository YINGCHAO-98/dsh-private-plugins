/**
 * dsh-plugin-manager host entry: mounts the plugin-manager HTTP routes once
    ...
    hostCtx.effect(
      () => {
        ...
        const snapshotManagedInstalled = async () =>
          selectManagedInstalled(snapshotInstalled(), await loadRepos(directory))
         ...
         */
        const startOperation = (kind, label, args, onSettled) => {
          if (activeOperation) {
        ...
        const disposeImportUpload = route(IMPORT_UPLOAD_PATH, async (req, res) => {
          ...
          try {
            ...
            const inbox = join(directory, '.dsh-plugin-manager', 'inbox')
        ...
        log('info', `dsh-plugin-manager ${VERSION} mounted for profile ${profile} (${directory})`)
