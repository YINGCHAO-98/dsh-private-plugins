...
export function gitCloneUrl(spec) {
  const url = gitUrlOf(spec).replace(/^git\+/, '')
  if (url.startsWith('github:')) {
    return `https://github.com/${url.slice('github:'.length)}.git`
  }
  return url
}
...
export function lockfileCommit(lockfileText, name) {
  ...
  // pnpm resolves github:owner/repo to a codeload tarball in the importer
  // section. The tarball suffix is the exact commit currently installed.
  const escapedName = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  const hostedRe = new RegExp(
    `(?:^|\\n)\\s+${escapedName}:\\s*\\n(?:\\s+[^\\n]*\\n){0,3}\\s+version:\\s+https://codeload\\.github\\.com/[^/\\s]+/[^/\\s]+/tar\\.gz/([0-9a-f]{7,40})(?:\\s|$)`,
    'm'
  )
  match = hostedRe.exec(lockfileText)
