/**
 * Private repository management for dsh-private-plugins.
 *
 ...
 *
 * The saved list lives in <profile>/.dsh-private-plugins/repos.json so the
...
/** A short human-readable name for a repository address or pnpm git spec. */
function repositoryName(value) {
  const base = value.split('#', 1)[0].replace(/\/+$/, '').replace(/\.git$/, '')
...
export function normalizeRepoInput(input) {
  ...
  let spec = raw
  const label = repositoryName(raw)
...
export function reposFilePath(profileDirectory) {
  return join(profileDirectory, '.dsh-private-plugins', REPOS_FILE)
}
