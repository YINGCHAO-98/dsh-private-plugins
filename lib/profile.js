/**
 * Profile resolution and manifest reads for dsh-private-plugins.
 *
...
export const SELF_NAME = 'dsh-private-plugins'
...
export function profileDir(profile, home = dshHome()) {
  if (!isDshProfileName(profile)) {
    throw new Error(`dsh-private-plugins: invalid profile name ${JSON.stringify(profile)}`)
  }
...
/** A git-style install spec (a private-repository cloud install). */
const GIT_SPEC_RE =
  /^(?:github|gitlab|bitbucket):[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+(?:#[^\s]+)?$|^git\+(?:https|ssh|file):\/\/[^\s\u0000;|&`$]+(?:#[^\s]+)?$|^git@[A-Za-z0-9_.:-]+:[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+(?:\.git)?(?:#[^\s]+)?$/

/**
 * True when a manifest dependency value is owned by this surface: a local
 * import (file:/link:/absolute path) or a cloud install from a git
 * repository (github:/gitlab:/bitbucket:, git+https/ssh/file, scp). npm
 * registry specs — community-market installs, which belong to dsh-market —
 * are never managed here.
 */
export function isManagedSpec(spec) {
  if (typeof spec !== 'string' || spec === '') return false
  if (spec.startsWith('file:') || spec.startsWith('link:') || spec.startsWith('/')) {
    return true
  }
  return GIT_SPEC_RE.test(spec)
}

/**
 * Plugins owned by this surface: local imports and cloud (git repository)
 * installs. The broader profile inventory also includes community-market
 * installs, which belong to dsh-market and must not be duplicated here —
 * those are npm registry specs and never pass isManagedSpec.
 */
export function selectManagedInstalled(installed, repos = []) {
  return installed.filter((plugin) => {
    ...
    return isManagedSpec(plugin.spec)
  })
}
